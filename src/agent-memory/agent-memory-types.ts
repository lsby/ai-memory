...
import { z } from 'zod'
import { type 智能体事件, type 智能体选项, type 解决问题选项 } from '../agent/types'
...
export let 快照数据Schema = z.object({ 记忆列表: z.array(记忆表Schema), 记忆关联列表: z.array(记忆关联表Schema) })
export type 快照数据 = z.infer<typeof 快照数据Schema>

export let 完整状态Schema = z.object({
  快照: z.string().optional(),
  消息历史: z.array(z.unknown()).optional().default([]),
})
